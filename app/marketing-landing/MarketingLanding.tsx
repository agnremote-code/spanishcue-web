...
export default async function MarketingLanding({ config }: { config: LandingConfig }) {
 ...
 const countryProofSlugs = new Set(['free-spanish-lesson','spanish-conversation-activities','ele-recursos-profesores','online-spanish-teaching-resources']);
 const countryProof = countryProofSlugs.has(config.slug) ? selected.filter(({lesson}) => lesson.id === 210 || lesson.id === 36) : [];
 const categoryLabels = locale === 'es' ? ['Gramática','Conversación','Escucha','Fonética','Vocabulario'] : ['Grammar','Conversation','Listening','Pronunciation','Vocabulary'];
 ...
 return <main className="landing-page campaign-page" id="landing-main">
  ...
  <section className="landing-products" id="real-lessons" aria-labelledby="landing-product-title"><header><span>{c.proof}</span><h2 id="landing-product-title">{c.proofTitle}</h2><p>{original.productCopy}</p></header><ul className="landing-category-strip" aria-label={locale === 'es' ? 'Categorías de la biblioteca' : 'Library categories'}>{categoryLabels.map(category => <li key={category}>{category}</li>)}</ul><div className="landing-lesson-grid">{selected.map(({lesson,path}) => {
  ...
  {countryProof.length === 2 && <section className="landing-country-proof" aria-labelledby="country-proof-title"><header><span>{locale === 'es' ? 'DOS CLASES INSIGNIA GRATIS' : 'TWO FREE FLAGSHIP LESSONS'}</span><h2 id="country-proof-title">{locale === 'es' ? 'UN MAPA PUEDE SER UNA CLASE ENTERA.' : 'ONE MAP. A FULL CONVERSATION LESSON.'}</h2><p>{locale === 'es' ? 'Elegí un lugar. Abrí una conversación. Enseñá. México ofrece 32 formas de vivir; Estados Unidos, 50.' : 'Choose a place. Open a conversation. Teach. Mexico offers 32 ways to live; the United States offers 50.'}</p></header><div>{countryProof.map(({lesson,path}) => <article key={lesson.id}><Image src={lesson.image} alt={lesson.title} width={720} height={450} loading="lazy" sizes="(max-width:760px) 88vw, 42vw"/><div><span>{lesson.id === 210 ? '32' : '50'} {locale === 'es' ? 'territorios' : 'territories'}</span><h3>{lesson.title}</h3><p>{lesson.subtitle}</p><LandingLink href={path} placement={`landing_country_proof_${config.slug}_${lesson.id}`}>{locale === 'es' ? 'ABRIR GRATIS' : 'OPEN FREE'} ↗</LandingLink></div></article>)}</div></section>}
  ...
  <footer className="landing-footer"><Link href="/"><SpanishCueBrand variant="compact" /></Link><nav aria-label={locale === 'es' ? 'Recursos, legal y contacto' : 'Resources, legal and contact'}><Link href="/resources">{locale === 'es' ? 'Recursos' : 'Resources'}</Link><Link href="/pricing">{locale === 'es' ? 'Precios' : 'Pricing'}</Link><Link href="/privacy">{locale === 'es' ? 'Privacidad' : 'Privacy'}</Link><Link href="/terms">{locale === 'es' ? 'Términos' : 'Terms'}</Link><Link href="/contact">{locale === 'es' ? 'Contacto' : 'Contact'}</Link></nav></footer>
 </main>;
}
