...
export default function ChooseConversation({variant="standard"}:{variant?:"standard"|"basic"|"starter"}={}){
 return <ConversationFamily id="lets-talk" title="Let’s Talk" levels={["A1","A2","B1","B2","C1"]} defaultLevel={variant==="starter"?"A1":variant==="basic"?"A2":"B1"}>{level=><TalkExperience key={level} variant={level==="A1"?"starter":level==="A2"?"basic":level==="B2"?"extended":level==="C1"?"nuanced":"standard"}/>}</ConversationFamily>;
}

function TalkExperience({variant}:{variant:"standard"|"basic"|"starter"|"extended"|"nuanced"}){
 const content=talkVariants[variant==="starter"?"A1":variant==="basic"?"A2":variant==="extended"?"B2":variant==="nuanced"?"C1":"B1"];
