...
export default function ArgentoRoleplays(){
  ...
  return <main className="arp-shell">
    ...
    {drawer&&<aside className="arp-drawer" role="dialog" aria-modal="true" aria-label={drawer==="slang"?"Slang argentino":"Ayuda para hablar"} onMouseDown={event=>{if(event.target===event.currentTarget)setDrawer(null)}}><div><header><section><h2>{drawer==="slang"?"🔥 ARGENTO":"🛟 AYUDA"}</h2><p>{drawer==="slang"?"Jerga argentina completa · Complete Argentine slang":"Frases para seguir hablando · Phrases to keep speaking"}</p></section><button onClick={()=>setDrawer(null)}>CERRAR · CLOSE ×</button></header>{drawer==="help"?<div className="arp-help">{helpGroups.map(group=><section key={group.title[0]}><PairText pair={group.title}/><div>{group.items.map(pair=><PairText pair={pair} key={pair[0]}/>)}</div></section>)}</div>:<div className="arp-slang">{slang.map(item=><article key={item.term}><header><h3>{item.term}</h3><span>{item.meaning}</span></header><p><b>Closest English slang:</b> {item.closest}</p><p>{item.use}</p><PairText pair={item.example}/>{item.note&&<small>{item.note}</small>}</article>)}</div>}</div></aside>}
  </main>;
}
