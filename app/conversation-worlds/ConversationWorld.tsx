...
export default function ConversationWorld({mode,level,machineRounds,ruleRounds,closing,guide}:{mode:'machine'|'rules';level:WorldLevel;machineRounds:WorldElimination[];ruleRounds:WorldRule[];closing:string[];guide?:WorldGuide}) {
  ...
  return <main className={`cw-world cw-${mode}${b2?' cw-b2':a1?' cw-a1':''}`} data-motion={motion}>
    <div className="cw-shell">
      <header className="cw-topbar"><Link href="/" className="cw-brand"><SpanishCueBrand variant="compact" context={b2||a1?'MUNDOS DE CONVERSACIÓN':'CONVERSATION WORLDS'} /></Link><Link href="/#library-results" className="cw-back">← Biblioteca</Link><div className="cw-top-actions"><span className="cw-level">{level} · CONVERSACIÓN</span>{a2&&<button onClick={()=>setShowEnglish(v=>!v)} aria-pressed={showEnglish}>{showEnglish?'Ocultar inglés':'Ayuda en inglés'}</button>}<button onClick={()=>setMotion(m=>!m)} aria-pressed={!motion}>{motion?'Pausar movimiento':'Activar movimiento'}</button></div></header>
      ...
      {(b2||a1)&&guide&&<section className="cw-b2-preparation" aria-labelledby="cw-prepare-title">
          ...
          {machine ? <div className={`cw-flip ${decision?'cw-flipped':''}`} key={entry.id}>
            <div className="cw-flip-inner">
              <article className="cw-card cw-front" aria-hidden={!!decision} inert={!!decision}>
                ...
                <h2>{item.title}</h2><p className="cw-context">{item.intro}</p><div className="cw-decision-prompt">{a1?'¿Lo eliminas o no?':b2?'¿Eliminarías esto del mundo?':'¿Lo eliminás para siempre?'}</div>
              ...
              <article className="cw-card cw-back-face" aria-hidden={!decision} inert={!decision}>
                {decision&&<><div className="cw-card-top"><span>TU PRIMERA DECISIÓN</span><span className={`cw-choice-pill cw-choice-${decision.first}`}>{decision.first==='yes'?'ELIMINAR':'CONSERVAR'}</span></div><h2 className="cw-item-heading">{item.title}</h2><p className="cw-eyebrow">AHORA, CONVERSEMOS</p><h3 className="cw-question" ref={questionRef} tabIndex={-1}>{item.question}</h3>{a2&&showEnglish&&<p className="cw-translation" lang="en">{item.questionEn}</p>}<>{a1?<BeginnerHelp frames={item.answerFrames??[item.starter]} words={item.words}/>:<TalkHelp words={item.words} starter={item.starter}/>}</>{b2&&item.followUp&&<details className="cw-help cw-b2-followup"><summary>Profundizar la conversación <span aria-hidden="true">+</span></summary><div><p>{item.followUp}</p></div></details>}
                {!decision.revealed?<button className="cw-primary cw-reveal" onClick={reveal}>{a1||a2?'¿Qué pasa después?':'Descubrir la consecuencia'} <span aria-hidden="true">↗</span></button>:<section className="cw-consequence"><p className="cw-eyebrow">GIRO · SI DESAPARECE…</p><p>{item.consequence}</p>{b2&&<div className="cw-b2-counterpoint"><p className="cw-eyebrow">OTRA VOZ</p><p>{item.counterpoint}</p><p className="cw-b2-revision">{item.revision}</p></div>}<h4>{a1?'¿Cambias de idea?':b2?'Después de escuchar la objeción…':a2?'¿Pensás lo mismo ahora?':'¿Seguís manteniendo tu decisión?'}</h4><div className="cw-final-choices"><button onClick={()=>decideFinal(false)} aria-pressed={decision.final===decision.first}>{a1?'No cambio de idea':a2?'Sí, pienso lo mismo':'Mantengo mi decisión'}</button><button onClick={()=>decideFinal(true)} aria-pressed={!!decision.final&&decision.final!==decision.first}>{a1?'Cambio de idea':a2?'No, cambio de idea':'Cambio de opinión'}</button></div>{decision.final&&<p className="cw-verdict">Decisión final: <strong>{decision.final==='yes'?'eliminar':'conservar'}</strong>. {a1?'Ahora elijo… porque…':b2?'Explica qué argumento aceptas, qué mantienes y bajo qué condición revisarías tu postura.':a2?'Contá por qué.':'Contá qué pesó más para vos.'}</p>}</section>}</>}
              </article>
            </div>
