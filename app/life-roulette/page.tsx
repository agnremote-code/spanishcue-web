...
export default function LifeRoulette(){
  ...
  if(active)return <main className="roulette-shell topic-mode" style={{"--topic":active.color} as RouletteStyle}>
    ...
    <nav className="roulette-nav"><Link href="/" className="roulette-brand"><span className="roulette-brand-mark" aria-hidden="true"><img src="/mascots/conversation.png" alt="" /></span><span><b>CHESPANISH</b><small>A2 · CONVERSACIÓN</small></span></Link><button onClick={back}>← VOLVER A LA RULETA</button></nav>
  ...
  return <main className="roulette-shell">
    <div className="roulette-atmosphere" aria-hidden="true"><i/><i/><i/></div>
    <nav className="roulette-nav"><Link href="/" className="roulette-brand"><span className="roulette-brand-mark" aria-hidden="true"><img src="/mascots/conversation.png" alt="" /></span><span><b>CHESPANISH</b><small>A2 · CONVERSACIÓN</small></span></Link><div className="roulette-nav-actions"><button className={sound?"sound-on":""} onClick={()=>setSound(value=>!value)} aria-pressed={sound}>{sound?"◉ SONIDO ON":"○ SONIDO OFF"}</button><Link href="/">← BIBLIOTECA</Link></div></nav>
    <section className="roulette-home">
      <header className="roulette-copy"><span className="roulette-kicker">🇦🇷 CONVERSACIÓN A2 · ARGENTINA</span><h1>LA RULETA<br/><em>DE TU VIDA</em></h1><b>HABLÁ · GIRÁ · CONECTÁ</b><p>17 temas potentes + 1 desafío argentino.<br/>Tres preguntas distintas en cada conversación.<small>17 strong topics + 1 Argentine challenge. Three different questions in every conversation.</small></p><div className="roulette-steps"><span><i>1</i>GIRÁ</span><b>→</b><span><i>2</i>DESCUBRÍ</span><b>→</b><span><i>3</i>HABLÁ</span></div><div className="roulette-actions"><button className="spin-main" onClick={random} disabled={spinning||!available.length}>{spinning?<><i className="button-spinner"/> GIRANDO…</>:<>🎡 GIRAR · SPIN</>}</button><button onClick={random} disabled={spinning||!available.length}>✦ SORPRENDEME</button><button onClick={()=>setLegend(true)}>VER LOS 18 ↗</button></div></header>
    ...
    {legend&&<div className="roulette-modal" role="dialog" aria-modal="true" aria-labelledby="roulette-legend-title" onMouseDown={()=>setLegend(false)}><section onMouseDown={event=>event.stopPropagation()}><button onClick={()=>setLegend(false)} aria-label="Cerrar">×</button><span>GUÍA RÁPIDA · QUICK GUIDE</span><h2 id="roulette-legend-title">17 temas + 1 desafío</h2><p>Elegí un símbolo directamente o dejá que la ruleta decida.</p><div>{topics.map(topic=><button key={topic.id} onClick={()=>{setLegend(false);pick(topic)}} disabled={used.includes(topic.id)}><b>{topic.icon}</b><span>{topic.es}<small>{topic.en}</small></span><i>{used.includes(topic.id)?"✓":"→"}</i></button>)}</div></section></div>}
  </main>;
}
