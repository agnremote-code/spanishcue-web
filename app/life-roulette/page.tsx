...
const Pair=({item}:{item:[string,string]})=><div className="roulette-pair"><b>{item[0]}</b><span>{item[1]}</span></div>;
...
export default function LifeRoulette(){
  ...
  const pick=(topic:Topic)=>{
    ...
    setRotation(next);
    window.setTimeout(()=>{setSelected(topic);setUsed(items=>[...items,topic.id]);setSpinning(false)},2800);
  };
  const random=()=>{if(!available.length)return;pick(available[Math.floor(Math.random()*available.length)])};
  const openTopic=(topic:Topic)=>{setActive(topic);setSelected(null);window.scrollTo({top:0,behavior:"smooth"})};
  const back=()=>{setActive(null);setSelected(null);window.scrollTo({top:0,behavior:"smooth"})};
  ...
  if(active)return <main className="roulette-shell topic-mode">
    ...
    <section className="topic-page">
      ...
      {active.id==="argento"?<section className="slang-board"><div className="topic-section-title"><span>🇦🇷 SIGNIFICADO + USO</span><h2>Modo Argento</h2><p>Tocá cada tarjeta para ver un ejemplo y cuándo usarla.</p><small>Tap each card to see an example and when to use it.</small></div><div className="slang-grid">{slangCards.map((item,index)=><button key={item[0]} className={openSlang.includes(index)?"open":""} onClick={()=>setOpenSlang(items=>items.includes(index)?items.filter(x=>x!==index):[...items,index])}><span>{openSlang.includes(index)?"−":"+"}</span><h3>{item[0]}</h3><b>{item[1]}</b>{openSlang.includes(index)&&<div><p><strong>Ejemplo:</strong> {item[2]}<small>{item[3]}</small></p><p><strong>Cuándo:</strong> {item[4]}<small>{item[5]}</small></p></div>}</button>)}</div><aside>💡 El tono cambia mucho el significado de algunas palabras argentinas.<small>Tone can greatly change the meaning of some Argentine words.</small></aside></section>:<div className="topic-board">
        <section className="questions-card"><span>3 PREGUNTAS · HABLÁ</span><h2>Tu turno.</h2>{active.questions.map((question,index)=><article key={question[0]}><i>{index+1}</i><div><b>{question[0]}</b><small>{question[1]}</small></div></article>)}<div className="followups"><span>¿Por qué?<small>Why?</small></span><span>¿Cuándo?<small>When?</small></span><span>¿Con quién?<small>With whom?</small></span></div></section>
        <aside className="support-stack"><section className="support-card yellow"><h3>EMPEZÁ ACÁ · START HERE</h3>{active.starters.map(item=><Pair key={item[0]} item={item}/>)}</section><section className="support-card"><h3>PALABRAS ÚTILES · USEFUL WORDS</h3>{active.words.map(item=><Pair key={item[0]} item={item}/>)}</section><section className="support-card pink"><h3>🇦🇷 ARGENTO · SLANG</h3>{active.slang.map(item=><Pair key={item[0]} item={item}/>)}</section></aside>
      </div>}
    ...
    <section className="roulette-home">
      ...
      <section className="wheel-zone" aria-label="Ruleta interactiva de 18 temas">
        ...
        <div className={`life-wheel ${spinning?"spinning":""}`} style={{transform:`rotate(${rotation}deg)`}}>
          {topics.map((topic,index)=><button key={topic.id} aria-label={topic.es} title={topic.es} className={`${used.includes(topic.id)?"used":""} ${selected?.id===topic.id?"selected":""}`} onClick={()=>pick(topic)} disabled={spinning||used.includes(topic.id)} style={{"--angle":`${index*20}deg`,"--counter":`${-rotation}deg`} as CSSProperties}>{topic.icon}</button>)}
        </div>
        <button className="wheel-hub" onClick={random} disabled={spinning||!available.length}><b>{spinning?"…":"GIRAR"}</b><span>SPIN</span></button>
        {selected&&<button className="wheel-result" onClick={()=>openTopic(selected)}><span>{selected.icon}</span><div><small>TEMA ELEGIDO · SELECTED TOPIC</small><b>{selected.es}</b><em>{selected.en}</em></div><strong>ABRIR →</strong></button>}
