...
import "../reino-unido-en-relieve/style.css";
import {areas,connectors,depthMoves,nationNames,starters,type Pair,type UKArea} from "./data";
import {ukShapes} from "../reino-unido-en-relieve/map-data";
...
const stancePairs:Record<Exclude<Stance,null>,Pair>={
  elegiria:{es:"Me gusta…",en:"I like…"},
  depende:{es:"Yo prefiero…",en:"I prefer…"},
  otra:{es:"Quiero…",en:"I want…"}
};
...
export default function ReinoUnidoConGus(){
  ...
  return <main className={`ch-app uk-app ${englishVisible?"":"ch-spanish-only"}`}>
    ...
    {screen==="cover"&&<section className="ch-cover uk-cover">
      ...
      <div className="ch-cover-copy">
        <div className="ch-level"><span>A1<br/>A2</span> EDICIÓN GUS · 100% CONVERSACIÓN</div>
        <p className="ch-eyebrow">4 NACIONES · 9 REGIONES INGLESAS · 48 LUGARES REALES · 144 PREGUNTAS BÁSICAS</p>
        <h1>REINO UNIDO<br/><em>EN RELIEVE</em></h1>
        <p className="ch-cover-lead">Un mapa del país que Gus conoce, hecho para hablar con español simple. <b>Cada territorio abre preguntas cortas sobre ciudades, costa, comida, música, viajes y vida real.</b><span className="ch-en">A map of the country Gus knows, designed for speaking with simple Spanish. Every area opens short questions about cities, coast, food, music, travel and real life.</span></p>
        ...
        <div className="ch-cover-stats"><article><b>12</b><span>territorios<small className="ch-en">mapped areas</small></span></article><article><b>48</b><span>lugares concretos<small className="ch-en">specific places</small></span></article><article><b>144</b><span>preguntas A1/A2<small className="ch-en">A1/A2 prompts</small></span></article></div>
      </div>
    ...
    {screen==="map"&&<section className="ch-map-page uk-map-page">
      ...
      <section className="ch-map-explorer">
        ...
        <aside className="ch-preview" style={{"--canton":mapPick.color} as CSSProperties}>
          ...
          <p>{mapPick.kind} · {nationNames[mapPick.nation].es}</p><h2>{mapPick.name}</h2><em>{mapPick.localName}</em><h3>{mapPick.hook.es}</h3><p className="ch-en">{mapPick.hook.en}</p><div className="ch-fact"><span>ALGO REAL · A REAL DETAIL</span><b>{mapPick.fact.es}</b><small className="ch-en">{mapPick.fact.en}</small></div><div className="ch-preview-places"><span>ADENTRO DEL TERRITORIO</span>{placesByCode[mapPick.code].map(place=><b key={place.name}>{place.name}</b>)}</div><div className="ch-preview-chips"><span>Centro: {mapPick.hub}</span><span>4 lugares</span><span>12 preguntas A1/A2</span></div><button onClick={()=>enter(mapPick)}>ENTRAR EN {mapPick.name.toUpperCase()} <span>→</span><small className="ch-en">OPEN THIS AREA</small></button>
        </aside>
      </section>
    ...
    {screen==="area"&&<section className="ch-canton-page uk-area-page" style={{"--canton":active.color} as CSSProperties}>
      <header className="ch-canton-hero">
        ...
        <div className="ch-canton-title"><small>{active.code} · {active.localName} · {active.languages}</small><h1>{active.name}</h1><h2>{active.hook.es}</h2><p className="ch-en">{active.hook.en}</p><div><span>4 lugares reales</span><span>8 preguntas centrales</span><span>Recursos bilingües</span></div></div>
      ...
      <div className="ch-classroom">
        ...
        <section className="ch-place-deck">
          <header><div><span>DENTRO DE {active.name.toUpperCase()} · INSIDE THIS AREA</span><h2>Cuatro lugares para hablar.</h2><p className="ch-en">Four places to talk about.</p></div><p>Tocá una parada, mirá el lugar y respondé con una frase simple.<small className="ch-en">Open a stop, look at the place and answer with one simple sentence.</small></p></header>
        ...
        <section className="ch-question-card">
          ...
          <div className="ch-question-copy"><small>{active.name} · {active.hook.es}</small><h2>{current.es}</h2><p className="ch-en">{current.en}</p><div><span>RESPONDÉ</span><i>→</i><span>USÁ PORQUE</span><i>→</i><span>AGREGÁ UN DETALLE</span></div></div>
          <UKMark/>
        </section>

        <section className="ch-response-grid">
          <article className="ch-stance"><span>1 · ELEGÍ UNA FRASE · CHOOSE A PHRASE</span><div>{(["elegiria","depende","otra"] as const).map(value=><button key={value} className={stance===value?"active":""} onClick={()=>setStance(value)}><b>{stancePairs[value].es}</b><small className="ch-en">{stancePairs[value].en}</small></button>)}</div></article>
        ...
        <section className="ch-answer-lab" aria-live="polite"><header><div><span>TU RESPUESTA A1/A2 · YOUR A1/A2 ANSWER</span><h2>Construí una frase y después decila sin leer.</h2></div><button disabled={!answerPairs.length} onClick={()=>{setStance(null);setAnswerParts([])}}>BORRAR · CLEAR</button></header><div className={`ch-answer-canvas ${answerPairs.length?"has-answer":""}`}>{answerPairs.length?answerPairs.map((part,index)=><button key={`${part.es}-${index}`} onClick={()=>{if(stance&&index===0)setStance(null);else{const partIndex=index-(stance?1:0);setAnswerParts(parts=>parts.filter((_,i)=>i!==partIndex))}}}><b>{part.es}</b><small className="ch-en">{part.en}</small></button>):<p><b>Tocá una frase, un comienzo y un conector.</b><span className="ch-en">Tap a phrase, a starter and a connector.</span></p>}</div>{answerPairs.length>0&&<div className="ch-answer-readout"><b>{answerEs}</b><span className="ch-en">{answerEn}</span></div>}</section>
        ...
        <section className="ch-depth"><header><span>HABLÁ UN POQUITO MÁS · SPEAK A LITTLE MORE</span><h2>Elegí una ayuda simple.</h2></header><div>{depthMoves.map((move,index)=><button key={move.es}><span>{index+1}</span><b>{move.es}</b><small className="ch-en">{move.en}</small></button>)}</div></section>
