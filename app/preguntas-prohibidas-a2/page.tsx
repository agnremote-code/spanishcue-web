...
export default function PreguntasProhibidasA2({initialLevel="A2"}:{initialLevel?:"A2"|"B1"}){
  return <ConversationFamily id="preguntas-prohibidas" title="El Reino de las Preguntas Prohibidas" levels={["A2","B1"]} defaultLevel={initialLevel}>{level=><KingdomExperience key={level} level={level as "A2"|"B1"}/>}</ConversationFamily>;
}

function KingdomExperience({level}:{level:"A2"|"B1"}){
  ...
  return <main className="pr-shell">
    <nav className="pr-nav"><Link href="/" className="pr-brand"><img src="/brand/mascot/portrait.webp" alt=""/><span><b>SPANISHCUE</b><small>{level} · CONVERSACIÓN</small></span></Link><div><span>{topics.length} MUNDOS</span><span>{topics.length*4} PREGUNTAS</span><span>{forbiddenBlocks.length} BLOQUES</span></div><Link href="/">← BIBLIOTECA</Link></nav>

    {screen==="cover"&&<section className="pr-cover">
      <div className="pr-cover-copy"><span className="pr-level">{level} · {content.focus}</span><p className="pr-overline">{content.focus}</p><h1>EL REINO DE LAS<br/><em>PREGUNTAS PROHIBIDAS</em></h1><p className="pr-lead">{content.intro}</p><p className="pr-lead-en">Explore the worlds, choose a question and answer with one idea, one reason and one example.</p><div className="pr-cover-actions"><button onClick={()=>show("rules")}>ENTRAR AL REINO <span>→</span></button><button onClick={()=>show("map")}>IR DIRECTO AL MAPA</button></div><div className="pr-cover-stats"><article><b>{topics.length}</b><span>temas cotidianos</span></article><article><b>{topics.length*4}</b><span>preguntas {level}</span></article><article><b>8</b><span>apoyos bilingües</span></article></div></div>
    ...
    {screen==="map"&&<section className="pr-page pr-map">
      ...
      <section className="pr-world-stage">
        ...
        <div className="pr-topic-grid">{topics.map((item,index)=><button className="pr-kingdom-gate" aria-label={`Abrir ${item.name}`} key={item.name} onClick={()=>openTopic(index)} style={{"--topic":item.color,"--gate-index":index} as CSSProperties}>
          ...
          <div className="pr-door-copy"><h3>{item.name}</h3><small className="pr-topic-english-name"></small><Pair value={item.desc}/><div className="pr-door-meta"><span>4 preguntas</span><b>ABRIR PUERTA →</b></div></div>
        </button>)}</div>
      </section>
    ...
    {screen==="topic"&&<section className="pr-page pr-topic-page" style={{"--topic":topic.color} as CSSProperties}>
      ...
      <section className="pr-topic-hero"><i>{topic.emoji}</i><div><small>MUNDO {level}</small><h1>{topic.name}</h1><span className="pr-topic-hero-en"></span><Pair value={topic.desc}/></div><b>4 PREGUNTAS</b></section>
