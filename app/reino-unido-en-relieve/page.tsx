  ...
  return <main className={`ch-app uk-app ${englishVisible?"":"ch-spanish-only"}`}>
    ...
    {screen==="cover"&&<section className="ch-cover uk-cover">
      ...
      <div className="ch-cover-copy">
        <div className="ch-level"><span>A2<br/>B1</span> 100% CONVERSACIÓN · 100% CONVERSATION</div>
        <p className="ch-eyebrow">4 NACIONES · 9 REGIONES INGLESAS · 48 LUGARES REALES · 144 DETONADORES</p>
        <h1>REINO UNIDO<br/><em>EN RELIEVE</em></h1>
        <p className="ch-cover-lead">No es una lista de “Londres, té y castillos”. <b>Es una expedición por barrios, costas, islas, ciudades industriales, lenguas y conexiones.</b> Cada lugar abre una pregunta que solamente tiene sentido en esa parte del mapa.<span className="ch-en">This is not a list of “London, tea and castles”. It is an expedition through neighbourhoods, coasts, islands, industrial cities, languages and connections. Every place opens a question that only makes sense in that part of the map.</span></p>
        ...
        <div className="ch-cover-stats"><article><b>12</b><span>territorios<small className="ch-en">mapped areas</small></span></article><article><b>48</b><span>lugares concretos<small className="ch-en">specific places</small></span></article><article><b>144</b><span>detonadores A2/B1<small className="ch-en">A2/B1 prompts</small></span></article></div>
      </div>
    ...
    {screen==="area"&&<section className="ch-canton-page uk-area-page" style={{"--canton":active.color} as CSSProperties}>
      ...
      <div className="ch-classroom">
        ...
        <section className="ch-place-deck">
          <header><div><span>DENTRO DE {active.name.toUpperCase()} · INSIDE THIS AREA</span><h2>Cuatro lugares que explican este territorio.</h2><p className="ch-en">Four places that explain this territory.</p></div><p>No son nombres decorativos: tocá cada parada, entendé su relación con la geografía local y usá la pregunta para hablar.<small className="ch-en">These are not decorative names: open each stop, understand its link to local geography and use its question to speak.</small></p></header>
        ...
        <section className="ch-answer-lab" aria-live="polite"><header><div><span>TU BORRADOR A2/B1 · YOUR A2/B1 DRAFT</span><h2>Armá una respuesta y después decila con tus palabras.</h2></div><button disabled={!answerPairs.length} onClick={()=>{setStance(null);setAnswerParts([])}}>BORRAR · CLEAR</button></header><div className={`ch-answer-canvas ${answerPairs.length?"has-answer":""}`}>{answerPairs.length?answerPairs.map((part,index)=><button key={`${part.es}-${index}`} onClick={()=>{if(stance&&index===0)setStance(null);else{const partIndex=index-(stance?1:0);setAnswerParts(parts=>parts.filter((_,i)=>i!==partIndex))}}}><b>{part.es}</b><small className="ch-en">{part.en}</small></button>):<p><b>Tocá una entrada, un comienzo y uno o dos conectores.</b><span className="ch-en">Tap an entry, a starter and one or two connectors.</span></p>}</div>{answerPairs.length>0&&<div className="ch-answer-readout"><b>{answerEs}</b><span className="ch-en">{answerEn}</span></div>}</section>
