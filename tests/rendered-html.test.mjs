...
test('library renders SPANISHCUE and excludes paid lesson bodies from public HTML',async()=>{
 ...
 assert.match(html,/Clase 01/);assert.match(html,/href="\/mexico"/);assert.match(html,/47 clases/);assert.match(html,/data-curriculum-order="100"/);
...
test('lesson cards expose real links and explain view results versus the complete catalog',async()=>{
 ...
 assert.match(html,/80(?:<!-- -->|\s)+resultados/);assert.match(html,/109(?:<!-- -->|\s)+clases totales/);
