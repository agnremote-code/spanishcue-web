...
test('library renders SPANISHCUE and excludes paid lesson bodies from public HTML',async()=>{
 ...
 assert.match(html,/Clase 01/);assert.match(html,/href="\/mexico"/);assert.match(html,/45 clases/);assert.match(html,/data-curriculum-order="100"/);
...
test('lesson cards expose real links and explain view results versus the complete catalog',async()=>{
 ...
 assert.match(html,/76(?:<!-- -->|\s)+resultados/);assert.match(html,/105(?:<!-- -->|\s)+clases totales/);
